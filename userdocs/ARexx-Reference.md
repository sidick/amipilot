...
| `PICK` | `[SCREEN=<substring>]` | Interactive "pick mode" discovery: hit-tests the LIVE global pointer position against `SCREEN=`'s windows (the frontmost screen if omitted) and returns the same `TREE`-shaped window line, plus (if any) a single gadget line for whichever gadget under the pointer also contains it — point at a gadget, get back its exact locator, no batch `TREE` dump required. `RC=5` if no window on the target screen contains the point at all. A window hit with no gadget hit is still `RC=0` (the window line alone — chrome/background, not an error). `AmiInspect PICK` is the standing-at-the-machine equivalent with no ARexx/wire session at all. See [Wire Protocol](Wire-Protocol.md#pick). |
